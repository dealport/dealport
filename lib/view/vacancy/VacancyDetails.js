...
function VacancyDetails(node, vacancy)
{
        ...
        if (this.isCreationConstructor(node))
        {
                ...
                this.appendChild(
                        ...
                        h4('', 'Location'),
                        this.location = new EditablePlainText(this.document, {
                                tagName: 'p',
...
VacancyDetails.prototype.attachEditingContexts = P.method(function(contextManager)
{
        return P.join(
                this.skills.attachEditingContext('skills', contextManager),
                this.location.attachEditingContext('location', contextManager),
                this.jobClassification.attachEditingContext('jobClassification', contextManager),
                this.salaryRange.attachEditingContext('salaryRange', contextManager),
                this.primaryRole.attachEditingContext('primaryRole', contextManager),
                this.secondaryRole.attachEditingContext('secondaryRole', contextManager)
        );
});