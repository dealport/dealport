...
function CompanyShortMetaTable(node, urlStateMap, company)
{
        ...
        if (this.isCreationConstructor(node))
        {
                ...
                this.addRow('revenueModel',
                        'Makes money via',
                        this.revenueModel = new EditablePlainText(this.document, {
                                tagName: 'div',
                                value: company.revenueModel
                        })
                );
...
CompanyShortMetaTable.prototype.attachEditingContexts = P.method(function(contextManager)
{
        return P.join(
                this.revenueModel.attachEditingContext('revenueModel', contextManager),
                this.hiring.attachEditingContext('hiring', contextManager),
                this.openForInvestment.attachEditingContext('openForInvestment', contextManager)
        );
});