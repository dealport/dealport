...
var CompanyGrid = require('../company/CompanyGrid');

require('static-reference')('./style/HomePage.less');

function HomePage(node, urlStateMap)
{
        ...
        if (this.isCreationConstructor(node))
        {
                ...
                this.appendChild(
                        div('intro',
                                ...
                                p('',
                                        ...
                                        this.textStateAnchors[this.textStateAnchors.length-1] =
                                                new StateAnchor(this.document, urlStateMap, ['page', 'home', 'submit'], '[Add Startup]')
                                ),
                                p('',
                                        'All the best, ',
                                        a({href: 'https://twitter.com/richardvdhorst'}, 'Richard')
                        ...
                        this.companyGrid = new CompanyGrid(this.document, urlStateMap)
                );
        }
        else
        {
                this.assertHasClass('HomePage');
                this.companyGrid = this.assertSelector('> .CompanyGrid', CompanyGrid, urlStateMap);
                this.textStateAnchors = this.selectorAll('> .intro .StateAnchor', StateAnchor, urlStateMap);
        }
}
...
Object.defineProperty(HomePage.prototype, 'editing', {
        configurable: true,
        get: function()
        {
        ...
        set: function(value)
        {
                ...
                this.companyGrid.editing = value;
        }
});

HomePage.prototype.attachEditingContexts = P.method(function(contextManager)
{
        return this.companyGrid.attachEditingContexts(contextManager);
});
