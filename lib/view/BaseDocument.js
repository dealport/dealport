...
function BaseDocument(node, urlStateMap)
{
        domv.HtmlDocument.call(this, node);

        if (!urlStateMap)
        {
                throw new domv.Exception(Error('Missing argument urlStateMap'));
        }
        ...
        if (this.isCreationConstructor(node))
        {
                ...
                this.appendChild(
                        this.fader = new StateAnchor(this.document, urlStateMap).cls('fader'),
                        this.header = div('header',
                                div('wrap',
                                        ...
                                        div('right',
                                                this.editButton = span('editButton', '[ edit ]'),
                                                ' ',
                                                this.userName = new StateAnchor(this.document, urlStateMap).cls('userName'),
                                                this.loginAnchor = new StateAnchor(this.document, urlStateMap, ['login'], 'login/register').cls('loginAnchor')
                                        )
                                )
                        ),
        ...
        else
        {
                ...
                this.fader = this.assertSelector('> .fader', StateAnchor, urlStateMap);
                ...
                this.editButton = this.header.assertSelector('> .wrap > div > .editButton');
                this.userName = this.header.assertSelector('> .wrap > div > .userName', StateAnchor, urlStateMap);
                this.loginAnchor = this.header.assertSelector('> .wrap > div > .loginAnchor', StateAnchor, urlStateMap);
        ...
        this.editButton.on('click', this._editButtonClick, false, this);
}
...
BaseDocument.prototype.setUser = function(user)
{
        ...
        this.userName.state = user && ['page', new this.urlStateMap.NamedEntity(user.namedEntityId || user._id), 'none'];
};
...
Object.defineProperty(BaseDocument.prototype, 'editButtonEnabled', {
        get: function()
        {
                return this.editButton.style.display !== 'none';
        },
        set: function(value)
        {
                this.editButton.style.display = value ? null : 'none';
        }
});

Object.defineProperty(BaseDocument.prototype, 'editState', {
        get: function()
        {
        ...
        set: function(value)
        {
                ...
                this.editButton.textContent = this._editState
                        ? '[ stop editing ]'
                        : '[ edit ]';
        }
});

BaseDocument.prototype.triggerEditStateChange = function(value)
{
        var ev = {
                baseDocument: this,
                editState: value
        ...
        if (this.editButton.emit('domv-edit-state', ev))
        {
                this.editState = value;
        }
};

BaseDocument.prototype._editButtonClick = function(e)
{
        this.triggerEditStateChange(!this.editState);
};