...
CompanyDetailPageController.prototype.enterEdit = function(state, upgrade)
{
        ...
        else
        {
                ...
                doc.contextualMenu.appendChild(
                        this.editAnchor = new StateAnchor(doc.document, router, this.getStateListFor('none'), '[ stop editing ]').cls('editAnchor')
                );
        }
...
CompanyDetailPageController.prototype._onEditChangeImpl = function(done)
{
        ...
        P.using(this.context.document.savePendingDisposer(), function()
        {
                ...
                var context = companyContexts.get(pageView.id);
                if (!context)
                {
                        console.warn('Unable to update company %s, the sharejs context is missing', pageView.id);
                }
                ...
                var op = Object.keys(values).map(function(key)
                {
                        ...
                        var od;
                        if      (typeof val === 'boolean') { od = false; }
                        else if (typeof val === 'string' ) { od = ''; }
                        else if (typeof val === 'number' ) { od = 0; }
                        else if (Array.isArray(val)      ) { od = []; }
                        else if (val === null            ) { od = null; }
                        else                               { od = {}; }
