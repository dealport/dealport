...
HomePageController.prototype.enterNone = function(state)
{
        ...
        var UserResource = this.context.resource.User;
        ...
        return UserResource.sessionUser().bind(this).then(function(user)
        {
                if (user)
                {
...
HomePageController.prototype.enterEdit = function(state)
{
        ...
        {
                this.parent.pageView.companyGrid.editing = true;
                doc.contextualMenu.appendChild(
                        this.editAnchor = new StateAnchor(doc.document, router, ['page', 'home', 'none'], '[ stop editing ]').cls('editAnchor')
                );
        }

        this.parent.pageView.companyGrid.on('domv-editable-change', this._onEditChange);
};

HomePageController.prototype.leaveEdit = function()
{
        this.parent.pageView.companyGrid.removeListener('domv-editable-change', this._onEditChange);
        this.parent.pageView.companyGrid.editing = false;
...
HomePageController.prototype._onEditChangeImpl = function(done)
{
        ...
        var items = this.parent.pageView.companyGrid.companies.filter(function(item)
        {
                // CompanyGridItem
                return item.lastChange > since;
        });

        P.map(items, function(item)
        {
                return Company.updateCompany(item.id, item.getValues());
        ...
        // todo show message fi no client side support
};
