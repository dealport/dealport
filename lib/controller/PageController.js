...
PageController.prototype.leave = function()
{
        ...
        P.all(this._acquiredContextsInState)
        .bind(this)
...
PageController.prototype._companyAcquireContexts = function(ids)
{
        ...
        {
                acquireContextResult.addListener(this._onCompanyOp, this);
...
PageController.prototype._userAcquireContexts = function(ids)
{
        var User = this.context.resource.User;

        var acquire = User.acquireContexts(ids);

        var p = acquire
                .bind(this)
                .then(function(acquireContextResult)
                {
                        acquireContextResult.addListener(this._onUserOp, this);
                        return acquireContextResult;
                });
...
{
        // todo really apply operations (instead of using the snapshot)
...
PageController.prototype._onUserOp = function(id, snapshot, components, keys)
{
        // todo really apply operations (instead of using the snapshot)
        ...
        if (this.currentState.type === 'NamedEntity' &&
           ...
           this.pageView.id === id)
        {
                console.info('Applying new data for keys ', keys, 'on user', id);
...
PageController.prototype.enterNamedEntity = function(namedEntityState, upgrade)
{
        ...
        if (upgrade)
        {
                ...
                if (this.pageView)
                {
                        ...
                        .then(function(acquireContextsResult)
                        {
                                this.child = new CompanyDetailPageController(
                                        this.context,
                                        this.pageView,
                                        acquireContextsResult.getSnapshot(this.pageView.id),
                                        acquireContextsResult
                                );
                        });
                }
                ...
                {
                        // user
                        return this._userAcquireContexts([this.pageView.id])
                                .bind(this)
                                .then(function(acquireContextsResult)
                                {
        ...
        .then(function(named)
        {
                ...
                if (named && named.company)
                {
                        ...
                        {
                                acquireContextP = this._companyAcquireContexts([named.company]);
                ...
                else if (named && named.user)
                {
                        if (this.context.server)
                        {
                                return [
                                        named,
                                        User.ids([named.user]).get(0),
                                        null // no editing context on the server
                                ];
                        }
                        else
                        {
                                acquireContextP = this._userAcquireContexts([named.user]);
