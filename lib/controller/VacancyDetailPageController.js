...
VacancyDetailPageController.prototype.enterRemove = function(state, upgrade)
{
        ...
        if (upgrade)
        {
                this.removeConfirmation = this.pageView.assertSelector('> .RemoveConfirmation', RemoveConfirmation, this.context.urlStateMap);
        }
        else
        {
                this.context.document.faderState = this.getFullStateList('none');

                this.removeConfirmation = new RemoveConfirmation(doc, this.context.urlStateMap, 'job vacancy', this.getFullStateList('none'));
...
VacancyDetailPageController.prototype._onRemoveConfirm = function(e)
{
        ...
        P.using(context.document.savePendingDisposer(), function()
        {
                ...
                .then(function()
                {
                        ...
                        return Company.ids([companyId], this._contextManager).get(0)
                        .then(function(company)
                        {
                                var namedEntity = new NamedEntityState(company.namedEntityId || company._id);
