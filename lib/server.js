...
function serverInitialize(config)
{
        ...
        //app.use('/favicon.ico', staticFile('lib/view/style/favicon.ico'));

        var cookie = cookieParser();
        app.use(cookie);
        primus.before('cookie', cookie);
        ...
        var session = expressSession({
                ...
                store: new MongoSessionStore(model)
        });
        ...
        app.use(function controllerRequest(req, res)
        {
                ...
                var context = new ClientContext(
                        true,
                        router,
                        new BaseDocument(document, router),
                        contextResources
                );
                ...
                {
                        return context.resource.User.sessionUser().then(function(user)
                        {
                                context.document.setUser(user);
                        });
        ...
        {
                spark.resources = resources(spark, model, config, spark.request.user);
        });
