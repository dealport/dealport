 ...
 *  "teaser": "Single line text",
 *  "description": "Multi line text",
 *  "jobClassification": "enum value, see job-classification.json",
 *  "salaryRange": { see salary-range.json },
 *  "primaryRole": "enum value, see job-role.json",
 *  "secondaryRole": "enum value, see job-role.json",
 *  "skills" : [
 *      "javascript",
 *      "less"
 *  ],
        ...
        if (comp.p.length === 1 && 'oi' in comp)
        {
                switch (comp.p[0])
                {
                        ...
                        case 'secondaryRole':
                                ...
                                {
                                        case 'jobClassification':
                                                allowedValues = jobClassificationData;
                                                break;
                                        case 'salaryRange':
                                                allowedValues = salaryRangeData;
                                                break;
                                        case 'primaryRole':
                                        case 'secondaryRole':
                                                allowedValues = jobRoleData;
                                                break;
                                }
                                ...
                                if ('oi' in comp)
                                {
                                        ...
                                        {
                                                if (equal(entry.value, comp.oi, {strict: true}))
                                                {
                                                        cleanComp.oi = comp.oi;
                                                }
                                        });
                                ...
                                if ('od' in comp)
                                {
                                        allowedValues.forEach(function(entry)
                                        {
                                                if (equal(entry.value, comp.od, {strict: true}))
                                                {
                                                        cleanComp.od = comp.od;
                                                }
                                        });
                                        ...
                                        {
                                                // make sure the key is not deleted (turn it into a replacement)
                                                cleanComp.oi = allowedValues[0].value;
                                        }
                                }
                                break;
                }
...
var vacancyDefaultFields = {
        title: 'Fill in a short title here',
        name: 'new-vacancy',
        company: null,
        teaser: 'A short one-line teaser for your vacancy',
        description: 'Describe your job vacancy in detail here...\n\n' +
                   'Maybe you could describe the atmosphere in your team...\n\n' +
                   'Or the passion that drives your company...',
        skills: ['list some', 'required skills', 'here'],
        location: 'Rotterdam',
        jobClassification: 'temporary',
...
VacancyModel.prototype.removeVacancy = function(id)
{
        var livedb = this.modelIndex.livedb;
        return livedb.submitAsync('vacancy', id, {del: true});
};