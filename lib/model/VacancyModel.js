...
VacancyModel.prototype.afterSubmit = P.method(function(docId, opData, snapshot, user)
{
        ...
        if (titleChanged)
        {
                return this.findUniqueName(snapshot.data.company, snapshot.data.title)
                .then(function(uniqueName)
                {
                        ...
                        console.info('generated an unique name for a vacancy',
                                docId, snapshot._v, snapshot.data.company, snapshot.data.title, uniqueName);
...
VacancyModel.prototype.findUniqueName = P.method(function(companyId, title)
{
        ...
        return P.using(this.modelIndex.acquire(), function(db)
        {
                ...
                for (var i = 1; i <= 25; ++i)
                {
                        ...
                        attempts.push(P.join(
                                nameAttempt,
                                db.vacancy.findOneAsync({company: companyId, name: nameAttempt}, {dummy: 1, _id: 0})
                        ));
                }
