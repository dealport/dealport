...
module.exports = function(config)
{
        ...
        P.join(
                ...
                // must be sparse because of the way livedb deletes stuff:
                initDb.vacancy.ensureIndexAsync({ company: 1, name: 1 }, { unique: true, sparse: true })
        )
        .then(function()
        {
                initDb.close();
        ...
        */
        ...
        // if a subscribe middleware is added, a bulk subscribe middleware must also
        // be implemented or otherwise sharejs triggers a "Not implemented" error
        share.use('bulk subscribe', function(request, callback)
        {
                // {collection name: {doc name -> version} }
                var promises = [];
                var collections = request.requests;
                var collectionNames = Object.keys(collections);

                collectionNames.forEach(function(collectionName)
                {
                        var docs = collections[collectionName];
                        var docNames = Object.keys(docs);

                        docNames.forEach(function(docName)
                        {
                                var version = docs[docName];

                                promises.push(models.sharejsSubscribe(
                                        collectionName,
                                        docName,
                                        version,
                                        request.agent
                ...
                .catch(function(err)
                {
                        console.warn('sharejs subscribe not allowed (bulk)', err, err.stack);
                        return P.reject(err);
                })
                .nodeify(callback);
        });
