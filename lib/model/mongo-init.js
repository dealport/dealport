...
module.exports = function(config)
{
        ...
        share.filter(function(collection, docId, docData, next)
        {
                ...
                .catch(function(err)
                {
                        console.warn('sharejs filter error', err);
                        return P.reject(err);
                })
                .nodeify(next);
        });
        ...
        share.filterOps(function(collection, docId, opData, next)
        {
                ...
                .catch(function(err)
                {
                        console.warn('sharejs filterOps error', err);
                        return P.reject(err);
                })
                .nodeify(next);
        });
        ...
        */
        ...
        share.use('submit', function(request, callback)
        {
                ...
                .catch(function(err)
                {
                        console.warn('sharejs submit not allowed', err);
                        return P.reject(err);
                })
                .nodeify(callback);
        });
        ...
        share.use('subscribe', function(request, callback)
        {
                ...
                {
                        console.warn('sharejs subscribe not allowed', err);
                        return P.reject(err);
                })
                .nodeify(callback);
        });
