 ...
 *  "name" : "{ New company! }",
 *  "logoUploadedImage": "object id hex string"
 *  "logoURL" : "https://example.com/foo.png", // not used in case logoUploadedImage is set
 ...
 *  "productDesc" : "long text with newlines",
 *  "visible" : false,
 *  "private" : {
 *      "editableByUser" : [
 *          ObjectId(...)
        ...
        if (comp.p.length === 1 && 'oi' in comp)
        {
                switch (comp.p[0])
                {
                        ...
                        case 'productDesc':
                                ...
                                break;
                        case 'homepage': // nullable string
                        case 'logoUploadedImage':
                                if ('oi' in comp)
                                {
...
CompanyModel.prototype.newCompany = P.method(function(initialFields)
{
        var fields = merge.recursive(companyDefaultFields, initialFields);
        var id = new ObjectID().toString();

        return this.modelIndex.livedb.submitAsync('company', id, {
                create: {
                        type: 'json0',
                        data: fields
                }
...
CompanyModel.prototype.removeCompany = P.method(function(companyId)
{
        ...
        return P.using(this.modelIndex.acquire(), function(db)
        {
                ...
                {
                        return livedb.submitAsync('vacancy', vacancy._id, {del: true});
                })
                .then(function()
                {
                        return db.namedEntity.removeAsync({company: companyId + ''});
                })
                .then(function()
                {
                        return livedb.submitAsync('company', companyId, {del: true});
                });
        });
});