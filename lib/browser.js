...
var initialHistoryState = null; // {url: '/foo'}
...
function pushHistoryState(state, newState)
{
        ...
        if (newState)
        {
                history.pushState({url: url}, context.document.title, url);
        }
        else
        {
                history.replaceState({url: url}, context.document.title, url);
        }
}
...
function doQueuedState()
{
        ...
        .done(function()
        {
                if (byUI)
                {
                        ...
                        gaPageView(urlStateMap.toURL(replacingStateWith || state));
...
{
        var state;
        if (!historyState || !historyState.url)
        {
                return false;
        }

        state = urlStateMap.fromURL(historyState.url);

        console.info('Entering', initial ? 'initial' : 'new', 'state from history', historyState.url, state);
...
function setupNavigation()
{
        ...
        global.onpopstate = function(e)
        {
                ...
                var p = handleHistoryState(e.state || initialHistoryState, false);
                if (p)
                {
