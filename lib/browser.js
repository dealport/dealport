...
function doQueuedState()
{
        ...
        var previousState = front.getChildrenStateList();
        ...
        statePromise.done(function()
        {
                if (byUI)
                {
                        ...
                        pushHistoryState(state, newState);
                        gaPageView(state);
                }
...
function primusInitialize(sessionID)
{
        return new P(function(resolve, reject)
        {
                ...
                {
                        console.log('Primus connected', sessionID);

                        primus.send('EHLO', {sessionID: sessionID}, function(data)
                        {
...
function loaded()
{
        primusInitialize(global.mySessionID).then(function(ehloData)
        {
                context = new ClientContext(false);
                context.router = router;
                context.document = new BaseDocument(document.documentElement, router);
                context.resource = contextResources;
                context.user = ehloData.user;
                context.sessionID = global.mySessionID;
                context.csrfToken = global.myCsrfToken;
