...
Resource.prototype._setupMultiplexing = function(channels)
{
        ...
        if (spark.primus) // server-side spark
        {
                ...
                for (i = 0; i < this.channelSparks.length; ++i)
                {
                        ...
                        if (!channel)
                        {
                                channel = primus.channel(channelName);
                        ...
                        Object.keys(channel.connections).forEach(function(key)
                        {
                                var channelSpark = channel.connections[key];
                                if (channelSpark)
                                {
                                        this._subscribeToChannel(channel, channelSpark);
                                }
                        }, this);
                        /* jshint +W083 */
                }
                ...
                // New subscriptions
                spark.on('subscribe', this._subscribeToChannel.bind(this));
...
Resource.prototype._subscribeToChannel = function(channel, channelSpark)
{
        var primus = this.primus;
        ...
        // make sure we are tracking it
        primus._ResourceChannels[channel.name] = channel;

        /*if (Resource.debugLogging)
        {
                console.log('Channel subscribe', channel.name, channelSpark.id);
        }*/
...
Resource.prototype.listen = function(names)
{
        ...
        names.forEach(function(name)
        {
                ...
                spark.on(fullname, this._rpcHandler(spark, name));
        }, this);
};
